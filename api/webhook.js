...
【輸出格式 - 嚴格遵守】
⚠️ 這是 LINE 純文字訊息，絕對禁止使用任何 Markdown 語法。
禁止：** 粗體 ** 、* 斜體 * 、# 標題、- 列表、數字+點 列表。
包含建議說明段落在內，整篇回覆都不能有任何 * 號或 # 號。
只能用純文字、emoji、換行排版。

第一段：今日狀態判斷（1-2行，直接說今天適不適合高強度，一句話說原因）

第二段：訓練菜單（每個動作單獨一行，格式如下）
🏋 動作名稱 | 重量 × 組數 × 次數 | 簡短提示

第三段：分隔線「———」後面再給訓練建議（今天為什麼這樣排、漸進邏輯、注意事項），可以寫長一點但不要廢話。建議段落用純文字分行，不要用數字列表或粗體。

範例格式：
今天睡眠充足，前一天休息，狀態 ✅ 可以衝。

🏋 坐姿划船（正手）| 17kg × 3組 × 12下 | 上次13下，今天目標穩定12下
🏋 輔助引體向上 | 輔助28.5kg × 3組 × 10下 | 嘗試比上次多2下
🏋 臀推 | 70kg × 3組 × 10下 | 上次66kg，今天微加重
🏋 壺鈴硬舉 | 20kg × 3組 × 12下 | 感覺太輕就最後一組加次數
🏋 高腳杯深蹲 | 12kg × 3組 × 10下 | 臀腿有訓練到就好

———
今天這樣排是因為昨天練了臀推，避免重複刺激同一肌群。划船重量上次穩定，今天目標加次數測試極限。引體向上輔助重量逐步減少，目標下次減2kg。注意核心要全程收緊。`;
